...
def fix_time(hour,minute,second,year,month,day):
    return "%04d-%02d-%02dT%02d:%02d:%02dZ" % (int(year),int(month),int(day),int(hour),int(minute),int(second))

def fix_coordinates(hemisphere,degrees,minutes):
    # Novatek stores coordinates in odd DDDmm.mmmm format
    minutes = float(minutes) / 10000.0
    coordinate = float(degrees)  + (minutes / 60.0)
    hemisphere = hemisphere.decode();
...
def fix_speed(speed):
    # 1 knot = 0.514444 m/s
    return float(speed) * float(0.514444)
...
def get_gps_atom(gps_atom_info,f):
    ...
    buf_size, = struct.unpack_from('>H', data, 16)
    buf = bytearray(buf_size)
    for i in range(buf_size):
        buf[i] = 0xaa ^ data[18+i]

    year, month, day, hour, minute, second = struct.unpack_from('>4s2s2s2s2s2s', buf, 8)
    sn, lat, lat_min, ew, lon, lon_min, speed = struct.unpack_from('>c2s6sc3s6s8s', buf, 38)

    time=fix_time(hour,minute,second,year,month,day)
    latitude=fix_coordinates(sn, lat, lat_min)
    longitude=fix_coordinates(ew, lon, lon_min)
    speed=fix_speed(speed)

    return (latitude,longitude,time,speed, 0)
